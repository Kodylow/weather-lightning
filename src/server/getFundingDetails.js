...
exports.getFundingDetails = async function getFundingDetails(options = {includeIndirectDeps: false}) {
...
exports.getLightningFundingDetails = async () => {
  let packages = [];
  const details = await exports.getFundingDetails();
  Object.keys(details.dependencies).forEach((dependency) => {
    packages.push({
      name: dependency,
      lnAddress: details.dependencies[dependency].funding.url
    });
  })
  return packages;
};